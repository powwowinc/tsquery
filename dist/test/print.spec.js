...
function foo(x, y) {
    ...
    return z;
        ...
        it('should handle synthetic nodes', () => {
            expect(print(factory.createArrowFunction([factory.createModifier(SyntaxKind.AsyncKeyword)], undefined, [
                factory.createParameterDeclaration(undefined, undefined, 'x', undefined, factory.createTypeReferenceNode('number'))
            ], undefined, factory.createToken(SyntaxKind.EqualsGreaterThanToken), factory.createBlock([])))).toEqual('async (x: number) => { }');
        });
    });
